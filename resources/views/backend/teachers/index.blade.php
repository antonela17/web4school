            ...
            @foreach ($teachers as $teacher)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    ...
                    <div class="w-2/12 flex items-center justify-end px-3">
                        <a href="{{ route('teachers.show',$teacher->id) }}" class="ml-1 bg-gray-600 block p-1 border border-gray-600 rounded-sm" title="assign subject">
...
<script type="text/javascript" src="https://code.jquery.com/jquery-1.7.1.min.js">
    $('#search').on('keyup',function(){
        ...
        objectData = {"name": name, "url":window.location.href};
        console.log(objectData)

                $.ajax({
                    type : 'get',
                    url : '{{route('search')}}',
                    data:{'search':name,'url':window.location.href},
                    error:function(data){
                        alert("ajhf")
                    }
                });
