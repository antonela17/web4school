        ...
        <div class="mt-8 bg-white rounded">
            <form action="{{--{{ route('profile.changepassword') }}--}}" method="POST" class="w-full max-w-2xl px-6 py-12">
                ...
                <div class="md:flex md:items-center mb-4">
                    ...
                    <div class="md:w-2/3">
                        <input name="newpassword" class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" pattern="^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$"
                               title="Password must contain one uppercase, one lowercase, one number, one special character and have at least 8 characters" type="password">
                        @error('newpassword')
